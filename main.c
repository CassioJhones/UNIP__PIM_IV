...
void visualizarIndicadores(const struct Empresa *empresa);
void excluirCadastro(const struct Empresa **empresas, int *numEmpresas);
...
int main() {
    ...
    while (telaAtual != TELA_SAIR) {
        ...
        switch (telaAtual) {
            ...
            case TELA_CADASTRO_EMPRESA:cadastrarEmpresa(&empresas, &numEmpresas, &telaAtual);break;
            case TELA_GERENCIAR_EMPRESAS:gerenciarEmpresas(empresas, numEmpresas, &telaAtual);break;
            case TELA_RELATORIO:exibirRelatorio(empresas, numEmpresas, &telaAtual);break;
            case TELA_CONSULTA_EMPRESA:{
                    ...
                    if (!empresaEncontrada) {printf(ANSI_RED"\nNenhuma empresa encontrada para o CNPJ informado.\n"ANSI_RESET);}
                    else {usuarioLogado.empresaSelecionada = indiceEmpresa;opGerenciamento(empresas, numEmpresas, indiceEmpresa, &telaAtual);}
                    printf("\nPressione Enter para continuar...");
                    while (getchar() != '\n');
                    telaAtual = TELA_MENU_PRINCIPAL;  } break;
            case TELA_NOVA_OPCAO:printf("Nova opção escolhida!\n");telaAtual = TELA_MENU_PRINCIPAL;break;}
        if (telaAtual == TELA_MENU_PRINCIPAL) {
            ...
            while (getchar() != '\n');
            switch (opcao) {
                case 1: telaAtual = TELA_CADASTRO_EMPRESA;break;
                case 2: telaAtual = TELA_GERENCIAR_EMPRESAS;break;
                case 3: telaAtual = TELA_RELATORIO;break;
                case 4: telaAtual = TELA_CONSULTA_EMPRESA;break;
                case 5: telaAtual = TELA_SAIR;break;
                default: printf(ANSI_RED"Opção inválida. Tente novamente.\n"ANSI_RESET);break;}}}
    free(empresas);printf("\n----Até Logo----\n");return 0;
}
struct Usuario fazerLogin(const struct Usuario *usuarios, int numUsuarios) {
...
void cadastrarEmpresa(struct Empresa **empresas, int *numEmpresas, int *telaAtual) {
    ...
    *empresas = temp;
    ...
    system("pause"); limparTela();*telaAtual = TELA_MENU_PRINCIPAL;
}
...
void gerenciarEmpresas(struct Empresa *empresas, int numEmpresas, int *telaAtual) {
    ...
    while (getchar() != '\n');
    switch (opcao) {
        case 1:*telaAtual = TELA_CONSULTA_EMPRESA;break;
        case 2:*telaAtual = TELA_MENU_PRINCIPAL;break;
...
void exibirRelatorio(const struct Empresa *empresas, int numEmpresas, int *telaAtual) {
    ...
    switch (opcaoRelatorio) {
        case 1:  {
            ...
            while (getchar() != '\n');
            switch (opcaoSalvar) {
                case 1:printf("Relatório salvo com sucesso!\n");break;
                case 2: *telaAtual = TELA_MENU_PRINCIPAL;break;
                default:printf("Opção inválida. Retornando ao menu anterior.\n");*telaAtual = TELA_MENU_PRINCIPAL;break;}
            break;}
            break;
        case 2: {
            printf("\n-------------------------------------");
            ...
            printf("Ano 1: ");scanf("%d", &ano1);
            printf("Mês 1 (1 a 12): ");scanf("%d", &mes1);
            printf("Ano 2: ");scanf("%d", &ano2);
            printf("Mês 2 (1 a 12): ");scanf("%d", &mes2);
            srand(time(NULL));
            ...
            printf("\nEm %s de %d, a empresa tratou %.2f toneladas de resíduos com um custo total de R$ %.2f.\n", obterNomeMes(mes1), ano1, toneladas1, custo1);
            printf("Enquanto em %s de %d tratou %.2f toneladas de resíduos com um custo total de R$ %.2f.\n", obterNomeMes(mes2), ano2, toneladas2, custo2);
            double aumentoToneladas = ((toneladas2 - toneladas1) / toneladas1) * 100.0;
            double aumentoCusto = ((custo2 - custo1) / custo1) * 100.0;
            printf("\nEm termos percentuais, temos um/uma %s de %.2f%% para a quantidade de resíduos e de %.2f%% para os custos totais.\n", (aumentoToneladas >= 0) ? "aumento" : "diminuição", fabs(aumentoToneladas), aumentoCusto);
            printf("\nOpções:\n");
            printf("[1] Salvar relatório\n");
            printf("[2] Voltar ao menu anterior\n");
            int opcaoSalvar;
            ...
            switch (opcaoSalvar) {
                case 1:printf("Relatório salvo com sucesso!\n");break;
                case 2:*telaAtual = TELA_GERAR_RELATORIO;break;
                default:printf(ANSI_RED"Opção inválida. Retornando ao menu anterior.\n"ANSI_RESET);*telaAtual = TELA_GERAR_RELATORIO;break;            }
            break;      }
case 3: {
    printf("\n-------------------------------------");
    ...
    if (ano1 < ANO_INICIAL || ano2 < ANO_INICIAL || ano1 > ANO_FINAL || ano2 > ANO_FINAL) {
        printf(ANSI_RED"Ano(s) inválido(s). Retornando ao menu anterior.\n"ANSI_RESET);
        *telaAtual = TELA_GERAR_RELATORIO;break;}
    srand(time(NULL));
    double toneladas1 = ((double)rand() / RAND_MAX) * 100.0;
    double custo1 = ((double)rand() / RAND_MAX) * 9000.0 + 1000.0;
    double toneladas2 = ((double)rand() / RAND_MAX) * 100.0;
    double custo2 = ((double)rand() / RAND_MAX) * 9000.0 + 1000.0;
    printf("\nNo ano %d, a empresa tratou %.2f toneladas de resíduos com um custo total de R$ %.2f.\n", ano1, toneladas1, custo1);
    printf("Enquanto em %d tratou %.2f toneladas de resíduos com um custo total de R$ %.2f.\n", ano2, toneladas2, custo2);
    double aumentoToneladas = ((toneladas2 - toneladas1) / toneladas1) * 100.0;
    double aumentoCusto = ((custo2 - custo1) / custo1) * 100.0;
    printf("\nEm termos percentuais, temos um/uma %s de %.2f%% para a quantidade de resíduos e de %.2f%% para os custos totais.\n", (aumentoToneladas >= 0) ? "aumento" : "diminuição", fabs(aumentoToneladas), aumentoCusto);
    printf("\nOpções:\n");
    ...
    switch (opcaoSalvar) {
        case 1: printf("Relatório salvo com sucesso!\n");break;
        case 2:*telaAtual = TELA_GERAR_RELATORIO;return;
        default:printf(ANSI_RED"Opção inválida. Retornando ao menu anterior.\n"ANSI_RESET);*telaAtual = TELA_GERAR_RELATORIO;break;
    }break;
}
break;
...
void relatorioEstadoMaiorVolume(const struct Empresa *empresas, int numEmpresas) {
    int anoDesejado;
    printf("Ano: ");
    scanf("%d", &anoDesejado);
    if (anoDesejado < ANO_INICIAL || anoDesejado > ANO_FINAL) {
...
void opGerenciamento(const struct Empresa *empresas, int numEmpresas, int empresaSelecionada, int *telaAtual) {
    ...
    while (getchar() != '\n');
    switch (opcao) {
        case 1: visualizarDadosCadastrais(&empresas[empresaSelecionada]);break;
        case 2: visualizarIndicadores(&empresas[empresaSelecionada]);break;
        case 4:excluirCadastro(&empresas, &numEmpresas);printf(ANSI_GREEN"\nCadastro excluído com sucesso!\n"ANSI_RESET); pausarExecucao();*telaAtual = TELA_MENU_PRINCIPAL;break;
        case 5: *telaAtual = TELA_GERENCIAR_EMPRESAS;break;
        case 6: *telaAtual = TELA_SAIR;break;
        default: printf(ANSI_RED"Opção inválida. Tente novamente.\n"ANSI_RESET);break;}
}
...
void excluirCadastro(const struct Empresa **empresas, int *numEmpresas){
    ...
    for (int i = 0; i < *numEmpresas; i++) {
        if (strcmp((*empresas)[i].cnpj, cnpjExclusao) == 0) {indiceExclusao = i;break;}}
    if (indiceExclusao != -1) {
        ...
        } else {printf(ANSI_RED"Erro ao alocar memória para a exclusão.\n"ANSI_RESET);}
    } else {printf(ANSI_RED"Empresa com o CNPJ fornecido não encontrada.\n"ANSI_RESET);}
}

void limparTela() {system("cls");}

void pausarExecucao() {printf(ANSI_GREEN"[---Pressione Enter para continuar---]"ANSI_RESET);while (getchar() != '\n');}
...
void visualizarIndicadores(const struct Empresa *empresa) {
    ...
    double valorArrecadadoAtual = 150000.0;
    double valorArrecadadoAnterior = 120000.0;
    printf(ANSI_GREEN"\n INDICADORES ATUAIS"ANSI_RESET);
    printf("\nNo dia %s a %s arrecadou R$ %.2f, ultrapassando a sua\n marca anterior de R$ %.2f.\n",dataAtual, empresa->nome_fantasia, valorArrecadadoAtual, valorArrecadadoAnterior);
}
